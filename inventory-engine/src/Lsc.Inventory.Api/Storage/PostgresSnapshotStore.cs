    ...
    ILogger<PostgresSnapshotStore> logger) : IInventorySnapshotStore
{
    ...
    public async Task<IReadOnlyList<StoredVehicleSnapshot>> GetCopartFutureBodyStyleCandidatesAsync(int maximum, CancellationToken cancellationToken)
    {
        await EnsureSchemaAsync(cancellationToken);
        var limit = Math.Clamp(maximum, 1, 100000);
        await using var connection = await OpenConnectionAsync(cancellationToken);
    ...
    public async Task<bool> UpdateCopartVehicleTypeAsync(string identity, DateTimeOffset expectedObservedAt, AuctionVehicle vehicle, CancellationToken cancellationToken)
    {
        if (!string.Equals(vehicle.Platform, "copart", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("Copart vehicle type backfill can only update Copart inventory.");
        if (string.IsNullOrWhiteSpace(vehicle.VehicleType)) return false;
        var observedAtUtc = expectedObservedAt.ToUniversalTime();
        var rawJson = JsonSerializer.Serialize(vehicle);
        var payloadHash = Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(rawJson))).ToLowerInvariant();
        var blobName = BuildBlobName(identity, observedAtUtc, payloadHash);

    ...
    ILogger<CopartExcelSnapshotProcessor> logger) : ICopartExcelSnapshotProcessor
{
    ...
    public async Task<CopartExcelProcessingResult> ProcessAsync(CopartSnapshotEnvelope snapshot, CancellationToken cancellationToken)
    {
        ...
        try
        {
            await foreach (var row in adapter.ReadAcceptedSnapshotAsync(snapshot, cancellationToken))
            {
                ...
                {
                    await ProcessBatchAsync(batch, observedLotKeys, state, registration.RunId!.Value, cancellationToken);
                    batch.Clear();
                }
            }
            if (batch.Count > 0)
            ...
            if (isComplete)
                reconciliation = await snapshotStore.ReconcileSourceAsync(InventorySourcePolicy.CopartExcelSource, observedLotKeys, true, snapshot.DownloadedAt, cancellationToken, registration.RunId!.Value);
            else
    ...
    private async Task ProcessBatchAsync(IReadOnlyList<AuctionVehicle> batch, ISet<string> observedLotKeys, ProcessingState state, Guid runId, CancellationToken cancellationToken)
    {
            ...
            try
            {
                ...
                if (!evaluation.LoadToSystem)
                {
                    ...
                    await snapshotStore.RecordSyncRunEventAsync(new InventorySyncRunEvent(
                        runId,
                        "copart",
                        $"copart:{evaluation.LotNumber ?? evaluation.VinMasked ?? "unknown"}",
                        evaluation.LotNumber,
                        evaluation.VinMasked,
                        evaluation.Decision == "CUARENTENA" ? "quarantined" : "discarded",
                        [],
                        evaluation.DiscardReasons.Concat(evaluation.Flags).Select(reason => reason.Code).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(),
                        DateTimeOffset.UtcNow), cancellationToken);
                    continue;
                }

                await snapshotStore.PersistAsync(vehicle, DateTimeOffset.UtcNow, cancellationToken, runId);
                state.Accepted++;

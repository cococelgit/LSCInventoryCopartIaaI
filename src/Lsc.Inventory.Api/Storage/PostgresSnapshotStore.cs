    ...
    IFacetsV2SharedCache? facetsV2SharedCache = null) : IInventorySnapshotStore, IAuctionsApiImportJobStore
{
    ...
    private static string SqlSellerTypeExpression(string alias) => SqlSellerTypeTaxonomy($"coalesce(case when lower(nullif(btrim({alias}.seller_type), '')) in ('unclassified', 'unknown', 'unavailable') then null else nullif(btrim({alias}.seller_type), '') end, nullif(btrim({alias}.payload #>> '{{seller,type}}'), ''), nullif(btrim({alias}.payload #>> '{{Seller,Type}}'), ''), nullif(btrim({alias}.payload #>> '{{seller,text_class}}'), ''), nullif(btrim({alias}.payload #>> '{{seller,textClass}}'), ''), nullif(btrim({alias}.payload #>> '{{Seller,TextClass}}'), ''), nullif(btrim({alias}.payload #>> '{{seller,class}}'), ''), nullif(btrim({alias}.payload #>> '{{Seller,Class}}'), ''), nullif(btrim({alias}.payload #>> '{{seller,name}}'), ''), nullif(btrim({alias}.payload #>> '{{Seller,Name}}'), ''))");
    ...
    public async Task<InventorySearchProjectionStatus> RebuildSearchProjectionAsync(CancellationToken cancellationToken)
    {
        ...
        await using (var finalize = connection.CreateCommand())
        {
            ...
            finalize.CommandText = """
                ...
                set is_ready = true, schema_version = 4, row_count = stats.rows, visible_row_count = stats.visible_rows, generated_at = stats.generated_at,

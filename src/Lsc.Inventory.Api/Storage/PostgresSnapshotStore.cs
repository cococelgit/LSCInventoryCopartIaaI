        ...
        await using (var rebuild = connection.CreateCommand())
        {
            ...
            rebuild.CommandText = """
                ...
                select lots.lot_key, lower(lots.platform), lots.lot_number, lots.vin, lots.title, lots.year, lots.make, lots.model,
                    ...
                    case when latest.payload #>> '{Seller,ClassificationConfidence}' ~ '^[0-9]+([.][0-9]+)?$' then (latest.payload #>> '{Seller,ClassificationConfidence}')::numeric end,
                    case lower(latest.payload #>> '{Seller,NeedsReview}') when 'false' then false when 'true' then true else true end,
                    latest.payload #>> '{Seller,ClassificationEvidence}', latest.payload #>> '{Seller,TaxonomyVersion}',

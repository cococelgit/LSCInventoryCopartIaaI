...
readonly IMAGE_TAG="inventory-api-integrated-r22-copart-media-https"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r20-active-summary"
...
readonly SOURCE_CONTEXT_URL="https://lsc-inv-revi-zyn4tlbw.manus.space/manus-storage/lsc-inventory-engine-api-r22-copart-media.tar_75bac64b.gz"
readonly SOURCE_CONTEXT_SHA256="235948bb936ae47d06acf65808a3f095b9bda07af01de05dc0c5999f7950ec10"

fail() { printf 'API_R22_DEPLOY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r22"
...
[[ "$copart_auto_template_after" == "$copart_auto_template_before" ]] || fail "Scheduled Copart auto job template changed unexpectedly"
[[ "$copart_auto_configuration_after" == "$copart_auto_configuration_before" ]] || fail "Scheduled Copart auto job configuration changed unexpectedly"
[[ "$copart_auto_identity_after" == "$copart_auto_identity_before" ]] || fail "Scheduled Copart auto job identity changed unexpectedly"
...
printf 'API_R22_DEPLOY_COMPLETED\n'

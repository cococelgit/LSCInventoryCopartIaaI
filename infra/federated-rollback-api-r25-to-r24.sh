#!/usr/bin/env bash
# Emergency rollback: changes only the shared API image from r25 back to r24.
set -Eeuo pipefail
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r25-rebuild-async"
readonly TARGET_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r24-copart-title-mapping"
...
fail() { printf 'API_R25_TO_R24_ROLLBACK_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r25 API image; found: ${api_image_before:-empty}"
[[ "$api_mode_before" == "Single" ]] || fail "Expected API Single revision mode; found: ${api_mode_before:-empty}"
[[ "$api_sync_before" == "false" && "$api_migrations_before" == "false" && "$api_warmup_before" == "true" ]] || fail "API environment guardrail failed"
[[ "$iaai_image_before" == "$EXPECTED_IAAI_IMAGE" && "$iaai_trigger_before" == "Schedule" && "$iaai_cron_before" == "$EXPECTED_IAAI_CRON" ]] || fail "IAAI guardrail failed"
[[ "$copart_auto_args_before" == "$EXPECTED_COPART_AUTO_ARGS" ]] || fail "Copart automatic guardrail failed"
[[ "$generic_trigger_before" == "Manual" ]] || fail "Generic job must remain Manual"

printf 'API_R25_TO_R24_ROLLBACK_START target=%s\n' "$TARGET_API_IMAGE"
az containerapp update --resource-group "$RESOURCE_GROUP" --name "$API_NAME" --image "$TARGET_API_IMAGE" --output none
...
[[ "$api_image_after" == "$TARGET_API_IMAGE" ]] || fail "API image was not rolled back to r24"
...
curl --fail --silent --show-error --retry 12 --retry-delay 5 "https://${api_fqdn}/healthz" >/dev/null || fail "r24 did not pass healthz after rollback"
printf 'API_R25_TO_R24_ROLLBACK_COMPLETED\nAPI_IMAGE=%s\nAPI_REVISION=%s\nAPI_READY_REVISION=%s\nIAAI_JOB_CHANGED=false\nCOPART_JOB_CHANGED=false\nCOPART_AUTO_JOB_CHANGED=false\nGENERIC_JOB_CHANGED=false\nPROJECTION_REBUILD_REVERSED=false\n' "$api_image_after" "$api_revision_after" "$api_ready_after"

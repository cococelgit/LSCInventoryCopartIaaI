#!/usr/bin/env bash
# Builds r13 and updates only the scheduled IAAI job. The schema initializer runs inside the job.
set -Eeuo pipefail
...
readonly IMAGE_TAG="iaai-national-schema-r13"
...
readonly EXPECTED_JOB_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:iaai-resilience-r12"
readonly EXPECTED_CRON="15,45 * * * *"
readonly SOURCE_CONTEXT_URL="https://lsc-inv-revi-zyn4tlbw.manus.space/manus-storage/lsc-inventory-engine-national-schema-r13.tar_32937f85.gz"
readonly SOURCE_CONTEXT_SHA256="fada14dcda38724284d2bf1111a08363e2fb36935612ae403795ba6bbd05e9a7"

fail() { printf 'IAAI_R13_DEPLOY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$(app_env_value Sync__Enabled)" == "false" ]] || fail "Legacy API sync must be disabled before r13"
[[ "$job_image_before" == "$EXPECTED_JOB_IMAGE" ]] || fail "Expected IAAI job r12 image; found: ${job_image_before:-empty}"
...
[[ "$job_image_after" == "$image_ref" ]] || fail "IAAI job image was not updated to r13"
...
printf 'IAAI_R13_DEPLOY_COMPLETED\n'

#!/usr/bin/env bash
# Builds the API revision with the protected, aggregated seller-taxonomy audit.
...
readonly IMAGE_TAG="inventory-api-integrated-r56-seller-taxonomy-audit"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r54-queue-claim-fairness"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r56-seller-taxonomy-audit"

fail() { printf 'API_R56_SELLER_TAXONOMY_AUDIT_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r54 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r56 seller taxonomy audit build"

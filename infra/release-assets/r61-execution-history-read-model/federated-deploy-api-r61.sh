#!/usr/bin/env bash
# Builds and deploys the execution-history read model correction.
...
readonly IMAGE_TAG="inventory-api-integrated-r61-execution-history-read-model"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r60-facets-v2-compact-sql"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r61-execution-history-read-model"

fail() { printf 'API_R61_EXECUTION_HISTORY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r60 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r61 execution-history build"

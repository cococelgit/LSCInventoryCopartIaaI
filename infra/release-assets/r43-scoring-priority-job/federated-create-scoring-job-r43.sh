...
fail() { printf 'SCORING_R43_PRIORITY_JOB_ABORT: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected API r42 image; found ${api_image_before:-empty}"
...
if az containerapp job show --resource-group "$RESOURCE_GROUP" --name "$SCORING_JOB_NAME" >/dev/null 2>&1; then
  ...
  if [[ "$scoring_image" != "$EXPECTED_API_IMAGE" ]]; then
    printf 'SCORING_R43_UPDATE_IMAGE_START old=%s new=%s\n' "$scoring_image" "$EXPECTED_API_IMAGE"
  ...
  printf 'SCORING_R43_ALREADY_CONFIGURED=true\n'
else
  printf 'SCORING_R43_CREATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"
...
printf 'SCORING_R43_CREATE_COMPLETED\n'

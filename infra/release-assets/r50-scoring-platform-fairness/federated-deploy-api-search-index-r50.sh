#!/usr/bin/env bash
# Builds the API revision that distributes historical scoring backfill across active platforms fairly.
...
readonly IMAGE_TAG="inventory-api-integrated-r50-platform-fairness"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-scoring-r50-platform-fairness"

fail() { printf 'API_R50_SCORING_PLATFORM_FAIRNESS_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r44 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r50 scoring platform fairness build"

#!/usr/bin/env bash
# Suppresses non-positive Buy Now values in the public API contract, updating only the Inventory API image.
...
readonly IMAGE_TAG="inventory-api-integrated-r70-buy-now-public-contract"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r69-buy-now-positive"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#fc193eb231499c27810c6ddfa05c6f064f63f8b8"

fail() { printf 'API_R70_BUY_NOW_PUBLIC_CONTRACT_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r69 Buy Now positive API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r70 Buy Now public contract build"

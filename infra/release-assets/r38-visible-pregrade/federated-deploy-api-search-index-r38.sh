#!/usr/bin/env bash
# Builds the API revision that keeps excludeSpecialTitles on the pre-grade fast path.
...
readonly IMAGE_TAG="inventory-api-integrated-r38-visible-pregrade"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r37-list-view-media"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r38-visible-pregrade"

fail() { printf 'API_R38_VISIBLE_PREGRADE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r37 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r38 visible pre-grade build"

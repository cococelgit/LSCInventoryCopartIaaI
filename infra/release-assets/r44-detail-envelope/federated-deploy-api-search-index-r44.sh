#!/usr/bin/env bash
# Builds the API revision that guarantees active vehicle details degrade safely when historical scoring data is incomplete.
...
readonly IMAGE_TAG="inventory-api-integrated-r44-detail-envelope"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r42-scoring-priority-audit"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-scoring-r44-detail-envelope"

fail() { printf 'API_R44_DETAIL_ENVELOPE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r42 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r44 detail envelope build"

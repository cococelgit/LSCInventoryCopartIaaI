#!/usr/bin/env bash
# Builds the API revision with priority scoring, durable execution audit, and resilient active detail pages.
...
readonly IMAGE_TAG="inventory-api-integrated-r42-scoring-priority-audit"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r41-detail-endpoint-fallback"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-scoring-r42-priority-audit"

fail() { printf 'API_R42_SCORING_PRIORITY_AUDIT_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r41 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r42 scoring priority audit build"

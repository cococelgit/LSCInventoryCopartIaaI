#!/usr/bin/env bash
# Builds the API revision with the protected seller-taxonomy audit query fix.
...
readonly IMAGE_TAG="inventory-api-integrated-r57-seller-taxonomy-audit-queryfix"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r56-seller-taxonomy-audit"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r57-seller-audit-queryfix"

fail() { printf 'API_R57_SELLER_TAXONOMY_AUDIT_QUERYFIX_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r56 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r57 seller taxonomy audit query fix build"

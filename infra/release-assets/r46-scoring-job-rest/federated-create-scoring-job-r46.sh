#!/usr/bin/env bash
# Creates only the scheduled LSC Pre-grade pilot using the Microsoft.App/jobs ARM resource.
...
fail() { printf 'SCORING_R46_REST_JOB_ABORT: %s\n' "$*" >&2; exit 1; }
...
if az containerapp job show --resource-group "$RESOURCE_GROUP" --name "$SCORING_JOB_NAME" >/dev/null 2>&1; then
  fail "Scoring job already exists; inspect its configuration before any replacement"
fi
...
manifest="/tmp/${SCORING_JOB_NAME}-r46.json"
jq -n \
  ...
  '{
    ...
    properties: {
      ...
      configuration: {
        ...
        identitySettings: [{ identity: $identity_id, lifecycle: "All" }]
      },
      template: {
        containers: [{
...
printf 'SCORING_R46_REST_PUT_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"

#!/usr/bin/env bash
# Makes title taxonomy coverage read from the active projection, updating only the Inventory API image.
...
readonly IMAGE_TAG="inventory-api-integrated-r67-title-coverage-projection"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r66-title-taxonomy-readonly"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#741b7d1facc529ace532622c24c989b6f22cfd37"

fail() { printf 'API_R67_TITLE_COVERAGE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r66 title taxonomy API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r67 title coverage build"

#!/usr/bin/env bash
# Creates the short-lived Facets V2 cache and updates only the Inventory API.
# It never updates or starts a job and never runs DDL, Sync, or a projection rebuild.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r63-facets-redis-shared-cache"
...
readonly REDIS_API_VERSION="2025-04-01"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r62-buy-now-range"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r63-facets-redis-shared-cache"

fail() { printf 'API_R63_FACETS_REDIS_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r62 API image; found: ${api_image_before:-empty}"
...
printf 'API_R63_UPDATE_START\n'
if ! az containerapp update --resource-group "$RESOURCE_GROUP" --name "$API_NAME" --image "$image_ref" --set-env-vars "FacetsRedis__Enabled=true" "FacetsRedis__Endpoint=${REDIS_ENDPOINT}" "FacetsRedis__ManagedIdentityClientId=${api_identity_client_id}" "FacetsRedis__KeyPrefix=lsc:facets:v2:" "FacetsRedis__TimeToLiveSeconds=15" "FacetsRedis__DistributedLockMilliseconds=4000" --output none; then
  printf 'API_R63_UPDATE_POLLING_RETRY: Azure CLI lost its operation-poll connection; verifying the submitted update directly.\n' >&2
  sleep 20
fi
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r63 Redis build"
[[ "$api_mode_after" == "$api_mode_before" && "$api_identity_after" == "$api_identity_before" && "$api_secrets_after" == "$api_secrets_before" && "$api_ingress_after" == "$api_ingress_before" && "$api_scale_after" == "$api_scale_before" && "$api_env_without_redis_after" == "$api_env_without_redis_before" && "$api_sync_after" == "$api_sync_before" && "$api_migrations_after" == "$api_migrations_before" && "$api_warmup_after" == "$api_warmup_before" ]] || fail "API configuration changed unexpectedly outside FacetsRedis"
[[ "$api_redis_enabled_after" == "true" && "$api_redis_endpoint_after" == "$REDIS_ENDPOINT" && "$api_redis_identity_after" == "$api_identity_client_id" ]] || fail "API Redis configuration was not applied"

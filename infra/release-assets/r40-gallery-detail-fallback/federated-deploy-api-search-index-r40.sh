#!/usr/bin/env bash
# Builds the API revision that returns every auction photo and recovers active detail pages from malformed full scoring data.
# This script changes only ca-lsc-inventory-api-prod and never starts or updates jobs.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r40-gallery-detail-fallback"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r39-full-gallery"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r40-gallery-detail-fallback"

fail() { printf 'API_R40_GALLERY_DETAIL_FALLBACK_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r39 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r40 gallery-detail fallback build"

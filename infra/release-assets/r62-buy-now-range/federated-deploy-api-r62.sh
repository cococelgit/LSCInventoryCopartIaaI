#!/usr/bin/env bash
# Builds and deploys the dedicated Buy Now range filter.
...
readonly IMAGE_TAG="inventory-api-integrated-r62-buy-now-range"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r61-execution-history-read-model"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r62-buy-now-range"

fail() { printf 'API_R62_BUY_NOW_RANGE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r61 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r62 Buy Now range build"

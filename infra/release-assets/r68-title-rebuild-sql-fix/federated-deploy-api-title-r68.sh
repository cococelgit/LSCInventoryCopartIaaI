#!/usr/bin/env bash
# Fixes the title rebuild SQL marker, updating only the Inventory API image.
...
readonly IMAGE_TAG="inventory-api-integrated-r68-title-rebuild-sql-fix"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r67-title-coverage-projection"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#34dc7b591494db60cc5cba8f17e5ebdeb61a88ff"

fail() { printf 'API_R68_TITLE_REBUILD_SQL_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r67 title coverage API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r68 title rebuild SQL build"

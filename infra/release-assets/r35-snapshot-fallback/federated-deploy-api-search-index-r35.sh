#!/usr/bin/env bash
# Builds the API revision that tolerates incompatible historical snapshot fields.
...
readonly IMAGE_TAG="inventory-api-integrated-r35-snapshot-fallback"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r34-copart-boolean"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r35-snapshot-fallback"

fail() { printf 'API_R35_SNAPSHOT_FALLBACK_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r34 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r35 snapshot fallback build"

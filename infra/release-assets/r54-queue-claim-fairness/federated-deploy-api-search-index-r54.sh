#!/usr/bin/env bash
# Builds the API revision that fairly claims historical scoring debt across platforms after reserving high priority.
...
readonly IMAGE_TAG="inventory-api-integrated-r54-queue-claim-fairness"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r52-visible-total-cache"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r54-queue-claim-fairness"

fail() { printf 'API_R54_QUEUE_CLAIM_FAIRNESS_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r52 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r54 queue claim fairness build"

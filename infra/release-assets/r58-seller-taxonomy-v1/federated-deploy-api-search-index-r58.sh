#!/usr/bin/env bash
# Builds the API revision with the canonical seller taxonomy and unclassified fallback.
...
readonly IMAGE_TAG="inventory-api-integrated-r58-seller-taxonomy-v1"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r57-seller-taxonomy-audit-queryfix"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r58-seller-taxonomy-v1"

fail() { printf 'API_R58_SELLER_TAXONOMY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r57 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r58 seller taxonomy build"

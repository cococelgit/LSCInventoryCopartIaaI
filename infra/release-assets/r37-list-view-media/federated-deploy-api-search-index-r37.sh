#!/usr/bin/env bash
# Builds the API revision that limits browse cards to one media item.
...
readonly IMAGE_TAG="inventory-api-integrated-r37-list-view-media"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r36-pregrade-baseline"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r37-list-view-media"

fail() { printf 'API_R37_LIST_VIEW_MEDIA_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r36 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r37 list-view media build"

...
fail() { printf 'SCORING_R48_BLOB_AUDIT_JOB_ABORT: %s\n' "$*" >&2; exit 1; }
...
manifest="/tmp/${SCORING_JOB_NAME}-r48.json"
jq -n \
...
printf 'SCORING_R48_BLOB_AUDIT_UPDATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"

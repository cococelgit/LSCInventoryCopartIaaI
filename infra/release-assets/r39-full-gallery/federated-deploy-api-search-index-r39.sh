#!/usr/bin/env bash
# Builds the API revision that returns every available auction photo to browse cards.
...
readonly IMAGE_TAG="inventory-api-integrated-r39-full-gallery"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r38-visible-pregrade"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r39-full-gallery"

fail() { printf 'API_R39_FULL_GALLERY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r38 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r39 full-gallery build"

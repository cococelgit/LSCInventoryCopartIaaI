...
fail() { printf 'SCORING_R51_PLATFORM_FAIRNESS_ABORT: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected API r50 image; found ${api_image_before:-empty}"
...
manifest="/tmp/${SCORING_JOB_NAME}-r51.json"
jq -n \
...
printf 'SCORING_R51_PLATFORM_FAIRNESS_UPDATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"

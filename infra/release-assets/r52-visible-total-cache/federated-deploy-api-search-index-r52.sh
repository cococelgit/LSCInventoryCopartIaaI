#!/usr/bin/env bash
# Builds the API revision that falls back safely when an older projection stores a stale visible-row cache.
...
readonly IMAGE_TAG="inventory-api-integrated-r52-visible-total-cache"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r50-platform-fairness"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r52-visible-total-cache"

fail() { printf 'API_R52_VISIBLE_TOTAL_CACHE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r50 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r52 visible total cache build"

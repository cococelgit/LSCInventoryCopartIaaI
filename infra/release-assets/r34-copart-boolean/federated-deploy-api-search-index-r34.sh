#!/usr/bin/env bash
# Builds the API revision that tolerates heterogeneous Copart boolean values.
...
readonly IMAGE_TAG="inventory-api-integrated-r34-copart-boolean"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r33-copart-deser"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r34-copart-boolean"

fail() { printf 'API_R34_COPART_BOOLEAN_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r33 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r34 Copart boolean build"

#!/usr/bin/env bash
# Builds the API revision that preserves active detail pages even when full scoring retrieval fails.
...
readonly IMAGE_TAG="inventory-api-integrated-r41-detail-endpoint-fallback"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r40-gallery-detail-fallback"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r41-detail-endpoint-fallback"

fail() { printf 'API_R41_DETAIL_ENDPOINT_FALLBACK_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r40 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r41 detail endpoint fallback build"

...
readonly EXPECTED_SCORING_JOB_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r44-detail-envelope"
...
fail() { printf 'SCORING_R53_PLATFORM_FAIRNESS_ABORT: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected API r52 image; found ${api_image_before:-empty}"
...
[[ "$(job_field "$SCORING_JOB_NAME" 'properties.template.containers[0].image')" == "$EXPECTED_SCORING_JOB_IMAGE" ]] \
  || fail "Existing scoring job image differs from r44; refusing to overwrite an unexpected update"
...
manifest="/tmp/${SCORING_JOB_NAME}-r53.json"
jq -n \
...
printf 'SCORING_R53_PLATFORM_FAIRNESS_UPDATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"

#!/usr/bin/env bash
# Builds and deploys the compact SQL optimization for the parallel Facets V2 endpoint.
...
readonly IMAGE_TAG="inventory-api-integrated-r60-facets-v2-compact-sql"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r59-facets-v2-shadow"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r60-facets-v2-compact-sql"

fail() { printf 'API_R60_FACETS_V2_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r59 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r60 compact Facets V2 build"

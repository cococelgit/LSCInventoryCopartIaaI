...
readonly EXPECTED_SCORING_JOB_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r52-visible-total-cache"
...
fail() { printf 'SCORING_R55_QUEUE_CLAIM_FAIRNESS_ABORT: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected API r54 image; found ${api_image_before:-empty}"
...
[[ "$(job_field "$SCORING_JOB_NAME" 'properties.template.containers[0].image')" == "$EXPECTED_SCORING_JOB_IMAGE" ]] \
  || fail "Existing scoring job image differs from r52; refusing to overwrite an unexpected update"
...
manifest="/tmp/${SCORING_JOB_NAME}-r55.json"
jq -n \
...
printf 'SCORING_R55_QUEUE_CLAIM_FAIRNESS_UPDATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"

#!/usr/bin/env bash
# Applies IAAI SPECIAL classification and makes title coverage read-only, updating only the Inventory API image.
...
readonly IMAGE_TAG="inventory-api-integrated-r66-title-taxonomy-readonly"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r64-facets-redis-diagnostics"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#3ec8218a1702ffa97be5453466b86ef0fa78afd6"

fail() { printf 'API_R66_TITLE_TAXONOMY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r63 Redis API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r66 title taxonomy build"

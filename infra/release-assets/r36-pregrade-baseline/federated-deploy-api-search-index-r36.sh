#!/usr/bin/env bash
# Builds the API revision with the indexed baseline Pre-grade browse path.
...
readonly IMAGE_TAG="inventory-api-integrated-r36-pregrade-baseline"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r35-snapshot-fallback"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-search-r36-pregrade-baseline"

fail() { printf 'API_R36_PREGRADE_BASELINE_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r35 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r36 pre-grade baseline build"

#!/usr/bin/env bash
# Builds and deploys the parallel Facets V2 endpoint in shadow mode.
# Changes only ca-lsc-inventory-api-prod. It never updates or starts any job and never runs DDL, Sync, or projection rebuilds.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r59-facets-v2-shadow"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r58-seller-taxonomy-v1"
readonly EXPECTED_IAAI_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:iaai-cursor-recovery-r14"
readonly EXPECTED_SCORING_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r52-visible-total-cache"
...
readonly SOURCE_CONTEXT_URL="https://github.com/cococelgit/LSCInventoryCopartIaaI.git#manus/api-r59-facets-v2"

fail() { printf 'API_R59_FACETS_V2_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected current r58 API image; found: ${api_image_before:-empty}"
...
[[ "$scoring_image_before" == "$EXPECTED_SCORING_IMAGE" ]] || fail "Expected scoring r52 image; found: ${scoring_image_before:-empty}"
[[ "$scoring_trigger_before" == "Schedule" && "$scoring_cron_before" == "$EXPECTED_SCORING_CRON" ]] || fail "Scoring trigger/cron changed unexpectedly"
[[ "$scoring_args_before" == "$EXPECTED_SCORING_ARGS" ]] || fail "Scoring arguments changed unexpectedly"

image_ref="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:${IMAGE_TAG}"
printf 'API_R59_BUILD_START image=%s git_context=%s\n' "$image_ref" "$SOURCE_CONTEXT_URL"
az acr build --registry "$REGISTRY_NAME" --image "${IMAGE_REPOSITORY}:${IMAGE_TAG}" --file Dockerfile "$SOURCE_CONTEXT_URL"
printf 'API_R59_UPDATE_START\n'
if ! az containerapp update --resource-group "$RESOURCE_GROUP" --name "$API_NAME" --image "$image_ref" --output none; then
  printf 'API_R59_UPDATE_POLLING_RETRY: Azure CLI lost its operation-poll connection; verifying the submitted update directly.\n' >&2
  sleep 20
fi
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r59 Facets V2 build"

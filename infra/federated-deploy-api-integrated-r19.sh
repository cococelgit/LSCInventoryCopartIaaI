...
readonly IMAGE_TAG="inventory-api-integrated-r19-active-inventory"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r18-platform-lot"
...
readonly SOURCE_CONTEXT_URL="https://lsc-inv-revi-zyn4tlbw.manus.space/manus-storage/lsc-inventory-engine-integrated-api-r19-final.tar_449cd6e5.gz"
readonly SOURCE_CONTEXT_SHA256="9e97bc449a964b019703174a91df2c1fd92017e292093bc6bbeb9dd37039f94d"

fail() { printf 'API_R19_DEPLOY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected integrated r18 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r19"

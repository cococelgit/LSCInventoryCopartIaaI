#!/usr/bin/env bash
# Builds the single integrated Inventory API release and updates only the shared API Container App.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r16"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:api-c4534db00aff42967f33411349fae469d3e83d5d"
...
readonly SOURCE_CONTEXT_URL="https://lsc-inv-revi-zyn4tlbw.manus.space/manus-storage/lsc-inventory-engine-integrated-api-r16.tar_8bf9ccd7.gz"
readonly SOURCE_CONTEXT_SHA256="3498536a0511eb782df2aeb38e66d6088c32ee9701cec2be37a8ef9f52c9bfa5"

fail() { printf 'API_R16_DEPLOY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected frozen Copart API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r16"
...
printf 'API_R16_DEPLOY_COMPLETED\n'

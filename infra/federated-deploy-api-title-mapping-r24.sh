#!/usr/bin/env bash
# Builds the Copart title mapping API revision and changes only the shared API Container App.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r24-copart-title-mapping"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r22-copart-media-https"
...
readonly SOURCE_CONTEXT_URL="https://lsc-inv-revi-zyn4tlbw.manus.space/manus-storage/lsc-inventory-engine-api-r24-title-mapping.tar_4377cc70.gz"
readonly SOURCE_CONTEXT_SHA256="e796f2c2e38d1b3557e1b016db9701b13dc1fc32af2debb012785c43b5526ef3"

fail() { printf 'API_R24_TITLE_MAPPING_DEPLOY_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r22 API image; found: ${api_image_before:-empty}"
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r24"
...
[[ "$api_scale_after" == "$api_scale_before" ]] || fail "API scale changed unexpectedly"
[[ "$api_sync_after" == "$api_sync_before" ]] || fail "API Sync__Enabled changed unexpectedly"
[[ "$api_migrations_after" == "$api_migrations_before" ]] || fail "API Persistence__RunMigrations changed unexpectedly"
[[ "$api_warmup_after" == "$api_warmup_before" ]] || fail "API search warmup setting changed unexpectedly"

#!/usr/bin/env bash
# Builds the integrated Run & Drive API revision with cached search statistics and changes only the shared API Container App.
set -Eeuo pipefail
...
readonly IMAGE_TAG="inventory-api-integrated-r29-search-index-buildfix"
...
readonly EXPECTED_API_IMAGE="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:inventory-api-integrated-r27-performance"
...
readonly SOURCE_CONTEXT_URL="https://raw.githubusercontent.com/cococelgit/LSCInventoryCopartIaaI/main/inventory-engine/infra/release-assets/r29-search-index-buildfix/lsc-inventory-engine-api-r29-search-index-buildfix.tar.gz"
readonly SOURCE_CONTEXT_SHA256="1a5722e48a6eb6f901b3261aead67282dd05d345a425fa5112c151718753ec4a"

fail() { printf 'API_R29_SEARCH_BUILDFIX_ERROR: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected r26 API image; found: ${api_image_before:-empty}"
...
temporary_source="$(mktemp --suffix=.tar.gz)"
build_context="$(mktemp -d)"
trap 'rm -f "$temporary_source"; rm -rf "$build_context"' EXIT
...
tar -xzf "$temporary_source" -C "$build_context"
[[ -f "$build_context/Dockerfile" && -f "$build_context/src/Lsc.Inventory.Api/Lsc.Inventory.Api.csproj" ]] || fail "Docker build context is incomplete"

image_ref="${REGISTRY_LOGIN_SERVER}/${IMAGE_REPOSITORY}:${IMAGE_TAG}"
printf 'API_R29_BUILD_START image=%s source_sha256=%s context=%s\n' "$image_ref" "$SOURCE_CONTEXT_SHA256" "$build_context"
az acr build --registry "$REGISTRY_NAME" --image "${IMAGE_REPOSITORY}:${IMAGE_TAG}" --file "$build_context/Dockerfile" "$build_context"
printf 'API_R29_UPDATE_START\n'
...
[[ "$api_image_after" == "$image_ref" ]] || fail "API image was not updated to r29 search buildfix"

#!/usr/bin/env bash
# Creates the isolated LSC Pre-grade job without changing the API, IAAI, or Copart jobs.
set -Eeuo pipefail
...
fail() { printf 'SCORING_R23_CREATE_ABORT: %s\n' "$*" >&2; exit 1; }
...
[[ "$api_image_before" == "$EXPECTED_API_IMAGE" ]] || fail "Expected API r22 image; found ${api_image_before:-empty}"
...
if az containerapp job show --resource-group "$RESOURCE_GROUP" --name "$SCORING_JOB_NAME" >/dev/null 2>&1; then
  ...
  [[ "$scoring_image" == "$EXPECTED_API_IMAGE" ]] || fail "Existing scoring job uses a different image"
  ...
  printf 'SCORING_R23_ALREADY_CONFIGURED=true\n'
else
  printf 'SCORING_R23_CREATE_START cron=%s maximum=%s\n' "$SCORING_CRON" "$SCORING_BATCH_MAXIMUM"
...
printf 'SCORING_R23_CREATE_COMPLETED\n'
